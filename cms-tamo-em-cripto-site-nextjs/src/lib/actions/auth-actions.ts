...
const schemaLogin = z.object({
	identifier: z
		...
		.max(100, { message: 'Please enter a valid username or email address' })
		.trim(),
...
export async function registerUserAction(prevState: any, formData: FormData) {
	...
	if (!validatedFields.success)
		return { errors: validatedFields.error.flatten().fieldErrors } //return { ...prevState, zodErrors: validatedFields.error.flatten().fieldErrors, strapiErrors: null, message: "Missing Fields. Failed to Register."}
	...
	if (!response)
		return { errors: 'Ops! Something went wrong. Please try again.' } // return { ...prevState, strapiErrors: null, zodErrors: null, message: "Ops! Something went wrong. Please try again."}
...
export async function loginUserAction(prevState: any, formData: FormData) {
	...
	// If any form fields are invalid, return early
	if (!validatedFields.success)
		return { errors: validatedFields.error.flatten().fieldErrors } //return {...prevState, zodErrors: validatedFields.error.flatten().fieldErrors, message: "Missing Fields. Failed to Login."}
	...
	if (!response)
		return { errors: 'Ops! Something went wrong. Please try again.' } // return { ...prevState, strapiErrors: responseData.error, zodErrors: null, message: "Ops! Something went wrong. Please try again."}
